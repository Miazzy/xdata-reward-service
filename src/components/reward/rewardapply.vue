              ...
              />
                ...
                <div class="reward-apply-content-item" style="margin-top:5px;margin-bottom:5px;">
                  <a-row>
                    ...
                    <a-col v-show="item.serialid" :span="4 * (iswechat?2:1)" style="font-size:1.0rem; margin-top:10px; text-align: center;">
                ...
                <div class="reward-apply-content-item" style="margin-top:5px;margin-bottom:5px; margin-right:10px;">
                  <a-row>
                    ...
                    <a-col :span="4 * (iswechat?2:1)" style="font-size:1.0rem; margin-top:10px; text-align: center;">
                ...
                <div class="reward-apply-content-item" style="position:relative; margin-top:5px;margin-bottom:5px; margin-right:10px;">
                  <van-cell-group :style="iswechat?`margin-left:1.25rem;width:95%;`:`margin-left:5rem;width:45%;`" >
                ...
                <div class="reward-apply-content-item reward-apply-content-title" :style="iswechat?`margin-right:0.25rem;`:`margin-right:8.75rem;`">
                  <a-row :style="iswechat?`border-top: 1px dash #f0f0f0;margin:0px 0.25rem;width:100%;height:auto;`:`border-top: 1px dash #f0f0f0;margin:0px 5rem;width:100%;height:auto;`" >
                    <vue-excel-editor v-model="data" ref="grid" width="100%" :page="30" :no-num-col="false" :readonly="false" :localized-label="vueExcelLabels" autocomplete @delete="onDelete" @update="onUpdate" >
      ...
      async handleApply() {
        ...
        this.$confirm({
            ...
            onOk: async() => {
                  ...
                  //获取到印章管理员组信息
                  let user_group_ids = response && response.length > 0 ? response[0].userlist + `,${userinfo.username}` : '';
                  let user_group_names = response && response.length > 0 ? response[0].enuserlist + `,${userinfo.username}` : '';

                  //如果未获取用户名称，则直接设置用印人为分组成员
                  if(tools.isNull(user_group_ids)){
                    user_group_ids = this.item.hr_id + `,${userinfo.username}`;
                    user_group_names = this.item.hr_name + `,${userinfo.username}`;
                  }
                  // 返回预览URL
